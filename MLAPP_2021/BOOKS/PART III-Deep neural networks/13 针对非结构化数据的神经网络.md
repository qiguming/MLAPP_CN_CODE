...
图13.3给出了一个包含两个隐藏层的MLP示意图，将该MLP应用于1.2.1.1节中的表格Iris数据集，该数据集具有4个特征和3个类别。 该模型具有如下形式
$$
\begin{align}
...
\end{align}
$$
其中 $\mathbf{\theta}=(\mathbf{W}_3,\mathbf{b}_3,\mathbf{W}_2,\mathbf{b}_2,\mathbf{W}_1,\mathbf{b}_1)$ 为模型中的参数，对应于3组可调节权重的边。我们看到最终（输出）层的激活函数与softmax函数相对应，softmax函数是分类分布的反向链接函数。对于隐藏层，我们可以自由选择激活函数所需的任何形式，如我们在第13.2.3节中所讨论的。

#### 13.2.4.2 MLP用于图像分类

要将MLP应用于图像分类，我们需要将2d输入“**展开**”（flatten）为1d向量。 然后，我们可以使用类似于第13.2.4.1节中所述的前馈网络。 例如，考虑构建一个MLP以对MNIST数字进行分类（第3.7.2节）。 这些数字是$28\times28 = 784$ 维向量。 要是我们使用2个各自具有128个单位的隐藏层，然后是最后的10路softmax层，我们得到的模型如图13.4所示。

我们在图13.5中显示了对该模型的一些预测。 我们对训练集仅训练两个“**周期**”（epochs, 遍历数据集的次数），但是该模型已经运行良好，测试集的准确率为97.1％。 此外，错误的预测案例似乎是可以理解的，例如将9误认为是3。训练更多的时间可以进一步提高测试的准确性。

在第14章中，我们讨论了另一种称为卷积神经网络的模型，该模型更适合于图像。 通过利用有关图像空间结构的先验知识，它可以获得更好的性能，并使用更少的参数。相比之下，MLP对输入的排列具有不变性。 换句话说，我们可以随机地对像素进行排列，并且可以获得相同的结果（假设我们对所有输入使用相同的随机排列）。

#### 13.2.4.3 MLP用于电影评论的情感分析

“文本分类的MNIST”可以说是来自[Maa + 11]的IMDB电影评论数据集。 （IMDB代表“互联网电影数据库”。）其中包含25k带有标签的示例用于训练，而25k用于测试。 每个示例都有一个二进制标签，代表肯定或否定的评分。 此任务称为（二进制）情感分析。 例如，这是训练集中的前两个示例：

1. this film was just brilliant casting location scenery story direction everyone’s
   really suited the part they played robert <UNK> is an amazing actor ...
2. big hair big boobs bad music and a giant safety pin these are the words to best
   describe this terrible movie i love cheesy horror movies and i’ve seen hundreds...

毫不奇怪，第一个示例被标记为正，第二个标记为负。

我们可以设计一个MLP来执行情感分析，如下所示。假设输入是一个包含$T$个符号的序列 $\mathbf{x}_{1:T}$，其中 $\mathbf{x}_t$ 是一个长度为 $V$ 的 one-hot 向量，其中 $V$ 为词库的大小。我们将此视为无序的单词袋（第10.4.3.1节）。模型的第一层为$E\times V$ 的嵌入矩阵 $\mathbf{W}_1$，该层将每一个稀疏 $V$ 维向量映射到一个稠密的 $E$ 维向量 ${\rm{e}}_t=\mathbf{W}_1\mathbf{x}_t$（见19.5节学习更多关于词嵌入的细节）。接着我们使用**全局平均池化**（global average pooling）将 $T\times D$ 的序列嵌入向量转化为一个固定长度的向量 ${\rm{\bar{\mathbf{e}}}}=\frac{1}{T}\sum_{t=1}^T{\rm{\mathbf{e}}}_t$。接着我们将该向量传入一个非线性隐藏层，计算一个 $K$ 维向量 $\mathbf{h}$，并将其传入最后的线性 logistic 层。 总结而言，模型定义如下：
$$
\begin{align}
...
\end{align}
$$
如果我们使用的词汇量为V = 1000，嵌入量为E = 16，隐藏层的量为16，则得到的模型如图13.6所示。 验证集的准确性为86％。

我们看到大多数参数都在嵌入矩阵中，这可能导致过度拟合。 幸运的是，正如我们在第19.5节中讨论的那样，我们可以执行单词嵌入模型的无监督预训练，然后我们只需微调此特定标记任务的输出层即可。

#### 13.2.4.4 MLP用于异方差回归

我们还可以使用MLP进行回归。 图13.7显示了如何为异方差 （heteroskedastic）非线性回归建立模型。 （术语“异方差”仅表示预测的输出方差与输入有关，如第3.3.3节中所述。）该函数具有两个输出，分别计算 $f_\mu(\mathbf{x})=\mathbb{E}[y|\mathbf{x},\mathbf{\theta}]$ 和 $f_\sigma(\mathbf{x})=\sqrt{\mathbb{V}[y|\mathbf{x},\mathbf{\theta}]}$。如图13.7所示，通过使用一个公共的“**主干网络**”（backbone）和两个输出“**头**”（heads）， 我们可以在这两个函数之间共享大多数层（因此也可以共享参数）。对于 $\mu$ 头，我们使用一个线性激活函数 $\varphi(a)=a$。对于 $\sigma$ 头，我们使用 softplus 激活函数 $\varphi(a)=\sigma_{+}(a)$。如果我们使用线性头和一个非线性主干网络，整个模型定义为：
$$
...
$$
图13.8显示了这种模型在数据集上的优势，在该数据集中，期望随时间线性增长，随季节波动，方差二次方增加。（这是一个**随机波动率模型** （stochastic volatility model）的简单示例；它可以用于对 财务数据以及 地球的全球温度进行建模，其中地球温度（由于气候变化）均值和方差不断增加。）我们看到 将输出方差 $\sigma^2$ 视为固定（与输入无关）参数的回归模型有时会置信度低，因为模型需要适应整体的噪声水平，并且无法适应输入空间中每个点的噪声水平。
